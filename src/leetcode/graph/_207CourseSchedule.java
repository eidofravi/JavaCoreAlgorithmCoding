...
public class _207CourseSchedule {
    ...
    public static void main(String[] args) {
        ...
        // int[][] prerequisites = {{552,312}, {552,538}, {538,42}, {312,106}, {220,42}};

        _207CourseSchedule schedule = new _207CourseSchedule();
        System.out.println(schedule.canFinish(2, prerequisites));
    }
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int[] prerequisite : prerequisites) {
            List<Integer> list = map.computeIfAbsent(prerequisite[0], t -> new ArrayList());
            list.add(prerequisite[1]);
        }
        Set<Integer> setNoLoopNumber = new HashSet<>();
        for (Integer key : map.keySet()) {
            canFinish = canFinish(setNoLoopNumber, map, key, new HashSet<>());
            if (!canFinish) {
                return false;
            }
        }
    ...
    public boolean canFinish(Set<Integer> setNoLoopNumber, Map<Integer, List<Integer>> map, int number, Set<Integer> navigated) {
        boolean canFinish = true;
        if (navigated.contains(number)) {
            return false;
        }
        List<Integer> listValues = map.get(number);
        if (null == listValues) {
            return true;
        }
        navigated.add(number);
        for (Integer value : listValues) {
            if (!setNoLoopNumber.contains(value)) {
                canFinish = canFinish(setNoLoopNumber, map, value, navigated);
                if (!canFinish) {
                    return canFinish;
                }
            }
        }
        setNoLoopNumber.add(number);
        navigated.remove(number);
        return canFinish;
    }
}
